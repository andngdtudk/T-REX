...
def mplight_mm(signals, w_bike=1.0, w_ped=1.0):
    ...
    w_bike / w_ped: relative weights. Pass via functools.partial when
    wiring into agt_config['reward'] so they're configurable per
    experiment without editing this file, e.g.:

        from functools import partial
        agt_config['reward'] = partial(mplight_mm, w_bike=1.0, w_ped=2.0)
    """
    rewards = dict()
    for signal_id in signals:
